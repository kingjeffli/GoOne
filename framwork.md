...
1. 当前仓库同时存在顶层 `api/` 与 `lib/api/`：前者是 proto source / generated code，后者是运行时 API 与上下文接口。
2. README 中提到的 `tools/protoc-gen-goone`、`tools/cmd/scaffold`、`tools/cmd/genproto` 现在都存在，且 `genproto` 已经是主仓推荐生成入口。
3. 协议与命令字以 `game_protocol` 子仓为共享 source-of-truth；主仓再基于它生成 `api/gen/**`，并逐步承接 protocol-owned service proto。
...
- 加载 `gamedata`
- 初始化 Redis
- 通过 IDL 生成的 `RegisterMainC2SServiceToDispatcher`（`ssrpc`）挂载主线命令（替代历史上分散的 `RegisterCmd` / `register.go`）
- 用 `RoleMgr` 管理角色对象
- 按分钟 tick 做角色级周期处理

特征：

- 使用 `globals.RoleMgr`；主线 handler 在 `src/mainsvr/service/c2s_ssrpc.go` 等中通过 `GetOrLoadRole` / `GetOrLoadOrCreateRole` 组合角色加载与业务逻辑
...
- 内网命令由 `ssrpc` 实现（如 `src/roomcentersvr/service/inner_ssrpc.go`）；按 `RouterID`（RID）选择分区/房间管理器的语义保留在业务层
...
- `controller.LoadWebRoutes` 中以 `RegisterWebApiServiceToDispatcher` + `MountGin` 挂载 IDL 生成路由；仍可通过 `RegisterLegacyWebRoutes` 保留少量兼容入口
...
- **推荐（当前主路径）**：在 `TransMgr.InitAndRun(...)` 之前，用 `ssrpc.NewDispatcher()` + `Register<Service>ToDispatcher(...)` + `d.RegisterToTransactionMgr(...)` 一次性挂上 IDL 生成的 `WrapUnary` handler；生成代码内部会调用 `mgr.RegisterCmd`，顺序要求不变。
- **遗留路径**：若仍存在手写 `RegisterCmd`，也必须发生在 `TransMgr.InitAndRun(...)` 之前。
- **handler 形态**：`ssrpc` 业务实现面对 `*ssrpc.Context` + 具体 req proto；底层仍由 runtime 封装为 `IContext` 与 `[]byte` 编解码。
- 有状态服务通常在 handler 内通过已有管理器取业务对象（如 `mainsvr` 的 `RoleMgr`，`roomcentersvr` 的房间列表/德州分区管理），避免在各处重复“如何加载角色/房间”的逻辑。
...
- 先确认命令字与 protobuf 消息定义在 `game_protocol` 中已经存在或应该落在那里。
- **默认**：修改 protocol-owned 或主仓 service proto 上的 `(goone.options.v1.ssrpc)`，再 `go run ./tools/cmd/genproto`，提交 `api/gen/**` 的生成差分。
- 历史上按服务维护的 `cmd_handler/register.go` 在当前树里已基本消失；除非明确仍存在 legacy 服务，否则不要新建这类分散注册。
...
5. 如果你要理解一个服务，优先读：
   - `main.go`
   - `app.go`（含 `Register*ToDispatcher` / `RegisterToTransactionMgr`）
   - `globals/globals.go`
   - 对应 `api/gen/**/<service>*_ssrpc.gen.go` 与 `src/<svr>/service/*` 实现
6. 如果你要改服务间通信，先读：
