...
状态：已完成（当前阶段确认不引入连接级队列，网关继续保持轻量收包转发，是否进一步队列化由压测和指标决定）
...
1. `tcp_impl.go` 在 `OnPacket()` 中直接 `go t.handler(conn, data)`，`ws_impl.go` 在 `OnRead()` 中通过 `safego.Go(...)` 异步投递。
2. `connsvr/pack_proc.go` 已把登录前置处理和默认转发拆开，网关主要承担拆包、少量校验和转发职责。
3. 包级并发仍然存在，但真正需要串行的状态修改已经更多依赖登录流程和下游事务模型保证。

影响：

1. 峰值流量下 goroutine 数仍会随包量上涨。
2. 如果后续继续把复杂状态逻辑堆到网关层，并发边界会再次变得模糊。
3. 当前更缺的是观测和压测依据，而不是立刻再引入一层连接级队列。

优化结论：

1. 现阶段不把网关改造成“单连接队列 + 单连接 worker”，避免在网关和业务层重复维护顺序语义。
2. 网关继续保持“轻前置处理 + 快速转发”，登录前置、重连绑定等关键路径单独控制。
3. 先补 `connsvr` 的 goroutine 数、收包速率、慢连接数、单连接处理耗时等指标，再根据压测结果决定是否推进连接级背压或队列化。

预期收益：

1. 优化重点继续集中在已经验证收益更高的事务调度、日志和加载路径。
2. 降低网关层改造复杂度，避免过早引入新的队列、背压和生命周期管理成本。
3. 如果后续指标证明网关成为瓶颈，再做连接级队列改造会更有依据。
...
1. `connsvr` 每秒收包数、活跃连接数、goroutine 数、慢连接数、单连接处理耗时。
