...
- **ssrpc 运行时**：`lib/service/ssrpc/*` -- IDL 驱动的统一 RPC runtime / codegen（当前 SSPacket/HTTP/WS 已接入，gRPC unary 可选）
...
| gRPC     | `WrapGRPCUnary`  | `grpc.Server`       | `grpc: true`             | runtime / codegen ready，主线未启用 |
...
- `ws` / `grpc` 当前仍要求有 cmd 绑定
- generator 当前只生成 gRPC unary 注册；streaming runtime helper 已有，但还未自动生成
- 当前主线业务 proto 还没有启用 `grpc: true`
---
